...
/**
 * BSC Oracle Price Updater
 * Fetches BNB/USD and P3D/USD prices from CoinGecko and updates the BSC oracle contract
 * 
...
class OracleBSCUpdater {
    constructor() {
        this.oracle = null;
        this.signer = null;
        this.bnbUsdPrice = null;
        this.p3dUsdPrice = null;
    }

    async initialize() {
        log('🚀 Initializing BSC Oracle Price Updater...', colors.cyan);
        
        try {
            // Setup provider and signer using BSC RPC
            const bscUrl = `https://bsc-dataseed.binance.org/`;
            const provider = new ethers.providers.JsonRpcProvider(bscUrl);
            log(`✅ Connected to BSC network`, colors.green);
    ...
    async checkBNBBalance() {
        try {
            log('💰 Checking BNB balance for signer...', colors.blue);
            
            // Get BNB balance
            const balance = await this.signer.getBalance();
            const balanceFormatted = ethers.utils.formatEther(balance);
            
            log(`   BNB Balance: ${balanceFormatted} BNB`, colors.blue);
            
            // Calculate required balance for transactions (3 price updates)
            // Using conservative gas estimates for BSC
            ...
            log(`   Required for transactions: ${totalRequiredFormatted} BNB`, colors.blue);
            
            if (balance.lt(totalRequired)) {
                const shortfall = totalRequired.sub(balance);
                const shortfallFormatted = ethers.utils.formatEther(shortfall);
                throw new Error(`Insufficient BNB balance. Need ${totalRequiredFormatted} BNB, have ${balanceFormatted} BNB. Shortfall: ${shortfallFormatted} BNB`);
            }
            
            log(`✅ Sufficient BNB balance for all transactions`, colors.green);
            return balance;
            
        } catch (error) {
            log(`❌ Failed to check BNB balance: ${error.message}`, colors.red);
            throw error;
        }
    }
    ...
    async fetchBNBPrice() {
        try {
            log('📊 Fetching BNB price from CoinGecko...', colors.blue);
            const bnbUsdPrice = await fetchCoingeckoExchangeRateCached('BNB', 'USD', true);
            
            this.bnbUsdPrice = bnbUsdPrice;
            TOKEN_CONFIGS.BNB.usdPrice = bnbUsdPrice;
            
            log(`✅ BNB/USD Price: $${bnbUsdPrice}`, colors.green);
            return bnbUsdPrice;
        } catch (error) {
            log(`❌ Failed to fetch BNB price: ${error.message}`, colors.red);
            throw error;
        }
    }
    ...
    async updateAllPrices() {
        try {
            ...
            // Fetch current BNB and P3D prices
            ...
            log(`   BNB/USD: $${this.bnbUsdPrice}`, colors.magenta);
            log(`   P3D/USD (adjusted): $${this.p3dUsdPrice} (raw price multiplied by ${EVMdecimalsMultiplier})`, colors.magenta);
            
            // Calculate price ratios
            const bnbToP3d = this.bnbUsdPrice / this.p3dUsdPrice;
            const p3dToBnb = this.p3dUsdPrice / this.bnbUsdPrice;
            
            log(`📊 Calculated Ratios (using adjusted P3D price):`, colors.magenta);
            log(`   BNB/P3D: ${bnbToP3d}`, colors.magenta);
            log(`   P3D/BNB: ${p3dToBnb}`, colors.magenta);
            log(`   P3D_PRECOMPILE/BNB: ${p3dToBnb} (same as P3D/BNB)`, colors.magenta);
            
            // Update _NATIVE_ vs P3D (BNB vs P3D)
            await this.updateOraclePrice('_NATIVE_', 'P3D', bnbToP3d, 1, '_NATIVE_ vs P3D (BNB vs P3D)');
            
            // Wait between transactions
            log('⏳ Waiting 3 seconds between transactions...', colors.yellow);
            await new Promise(resolve => setTimeout(resolve, 3000));
            
            // Update P3D vs _NATIVE_ (P3D vs BNB)
            await this.updateOraclePrice('P3D', '_NATIVE_', p3dToBnb, 1, 'P3D vs _NATIVE_ (P3D vs BNB)');
            ...
            await this.updateOraclePrice(TOKEN_CONFIGS.P3D.P3D_PRECOMPILE, '_NATIVE_', p3dToBnb, 1, 'P3D Precompile vs _NATIVE_ (P3D Precompile vs BNB)');
    ...
    async run() {
        try {
            ...
            log('🎉 BSC Oracle price update process completed successfully!', colors.green);
...
if (require.main === module) {
    const updater = new OracleBSCUpdater();
    updater.run()
        .then(() => {
            log('✅ BSC Oracle price updater completed successfully', colors.green);
            process.exit(0);
        })
        .catch(error => {
            log(`❌ BSC Oracle price updater failed: ${error.message}`, colors.red);
            console.error(error);
            process.exit(1);
        });
}

module.exports = OracleBSCUpdater;
