...
contract ImportWrapperAssistant is ERC20, ReentrancyGuard, CounterstakeReceiver, ERC165 {
	...
	address public precompileAddress; // The precompile address for the image asset
	...
	constructor(address bridgeAddr, address managerAddr, uint16 _management_fee10000, uint16 _success_fee10000, uint16 _swap_fee10000, uint8 _exponent, string memory name, string memory symbol) ERC20(name, symbol) {
		initImportWrapperAssistant(bridgeAddr, managerAddr, _management_fee10000, _success_fee10000, _swap_fee10000, _exponent, name, symbol);
	}

	function initImportWrapperAssistant(address bridgeAddr, address managerAddr, uint16 _management_fee10000, uint16 _success_fee10000, uint16 _swap_fee10000, uint8 _exponent, string memory _name, string memory _symbol) public {
		...
		// Get settings from ImportWrapper
		(address tokenAddr, , , , , ) = ImportWrapper(bridgeAddr).settings();
		tokenAddress = tokenAddr;
		
		// Get the precompile address from the ImportWrapper
		precompileAddress = ImportWrapper(bridgeAddr).precompileAddress();
	...
	// Approve bridge to spend this assistant's ERC20 precompile stake token.
	// - Manager-only
	...
	function getGrossBalance() internal view returns (UintBalance memory bal) {
		...
		if (tokenAddress == Counterstake(bridgeAddress).P3D_PRECOMPILE()) {
			...
			stake_bal = IPrecompileERC20(tokenAddress).balanceOf(address(this));
		}

		uint image_bal = IPrecompileERC20(precompileAddress).balanceOf(address(this));
	...
	function claim(string memory txid, uint32 txts, uint amount, int reward, string memory sender_address, address payable recipient_address, string memory data) onlyManager nonReentrant external {
		...
		uint claim_num = ImportWrapper(bridgeAddress).last_claim_num() + 1;
		...
		ImportWrapper(bridgeAddress).claim{value: 0}(txid, txts, amount, reward, required_stake, sender_address, recipient_address, data);

		(uint num, uint den) = getOraclePriceOfNative(); // price of P3D in terms of stake token
	...
	function challenge(uint claim_num, CounterstakeLibrary.Side stake_on, uint stake) onlyManager nonReentrant external {
		...
		uint missing_stake = ImportWrapper(bridgeAddress).getMissingStake(claim_num, stake_on);
		...
		ImportWrapper(bridgeAddress).challenge{value: 0}(claim_num, stake_on, stake);
	...
	function recordLoss(uint claim_num) nonReentrant external {
		...
		CounterstakeLibrary.Claim memory c = ImportWrapper(bridgeAddress).getClaim(claim_num);
		...
		uint my_winning_stake = ImportWrapper(bridgeAddress).stakes(claim_num, c.current_outcome, address(this));
		require(my_winning_stake == 0, "have a winning stake in this claim");
		
		uint my_losing_stake = ImportWrapper(bridgeAddress).stakes(claim_num, opposite_outcome, address(this));
	...
	function buyShares(uint stake_asset_amount, uint image_asset_amount) payable nonReentrant external {
		...
		// Transfer image assets from the precompile
		if (precompileAddress != address(0) && precompileAddress != Counterstake(bridgeAddress).P3D_PRECOMPILE()) {
			require(IPrecompileERC20(precompileAddress).transferFrom(msg.sender, address(this), image_asset_amount), "failed to pull image from precompile");
		}
	...
	function getOraclePriceOfNative() view private returns (uint, uint) {
		if (tokenAddress == Counterstake(bridgeAddress).P3D_PRECOMPILE())
			return (1, 1);
		address oracleAddr = ImportWrapper(bridgeAddress).oracleAddress();
	...
	function payImageTokens(address to, uint amount) internal {
		// Transfer image tokens from the precompile
		if (precompileAddress != address(0) && precompileAddress != Counterstake(bridgeAddress).P3D_PRECOMPILE()) {
			require(IPrecompileERC20(precompileAddress).transfer(to, amount), "failed to transfer image asset from precompile");
		}
	}
