...
library CounterstakeLibrary {
	...
	// Helper function to get symbol from precompiles and regular ERC20 tokens
	function getPrecompileSymbol(address tokenAddr) internal view returns (string memory) {
		if (tokenAddr == P3D_PRECOMPILE) {
			// P3D precompile - use IP3D interface
			return IP3D(tokenAddr).symbol();
		} else if (is3DPassERC20Precompile(tokenAddr)) {
			// Other 3DPass ERC20 precompiles - use IPrecompileERC20 interface
			return IPrecompileERC20(tokenAddr).symbol();
		} else {
			// Regular ERC20 tokens - use IERC20WithSymbol interface
			return IERC20WithSymbol(tokenAddr).symbol();
		}
	}

	// Approve bridge to spend ERC20 precompile tokens.
	// Not applicable to P3D precompile.
	function approvePrecompile(address tokenAddr, address bridgeAddr) internal {
		require(tokenAddr != P3D_PRECOMPILE, "not for P3D precompile");
		require(is3DPassERC20Precompile(tokenAddr), "not ERC20 precompile");
		IPrecompileERC20(tokenAddr).approve(bridgeAddr, type(uint).max);
	}
	...
	function challenge(
		...
		if (excess > 0){
			if (isP3D(settings.tokenAddress))
				require(IP3D(settings.tokenAddress).transfer(msg.sender, excess), "P3D transfer failed");
			else if (is3DPassERC20Precompile(settings.tokenAddress))
				require(IPrecompileERC20(settings.tokenAddress).transfer(msg.sender, excess), "3DPass ERC20 transfer failed");
			else
				revert("unsupported token type for excess refund");
		}
	}
