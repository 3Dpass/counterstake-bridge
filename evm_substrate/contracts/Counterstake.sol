...
abstract contract Counterstake is ReentrancyGuard {
	...
	function claim(string memory txid, uint32 txts, uint amount, int reward, uint stake, string memory sender_address, address payable recipient_address, string memory data) nonReentrant payable external {
		if (recipient_address == address(0)) // default input value
	...
	function withdraw(uint claim_num, address payable to_address) nonReentrant public {
		if (to_address == address(0)) // default input value
...
//	event ExternalCall(bool res, string errtype, uint initial_gas, uint gas, address payment_recipient_address);
	...
	// Validation function for token addresses - 3DPass only
	function validateTokenAddress(address _tokenAddr) internal pure {
		// Allow P3D precompile address for native tokens on 3DPass
		if (_tokenAddr == P3D_PRECOMPILE) {
			return;
		}
		// Allow 3DPass ERC20 precompile addresses
		if (is3DPassERC20Precompile(_tokenAddr)) {
			return;
		}
		// Reject all other addresses including address(0)
		revert("unsupported token type: only P3D and 3DPass ERC20 precompiles allowed");
	}
	...
	// P3D and 3DPass ERC20 precompile-aware transfer functions
	function transferTokens(address token, address to, uint256 amount) internal {
		...
		} else if (is3DPassERC20Precompile(token)) {
			require(IPrecompileERC20(token).transfer(to, amount), "3DPass ERC20 transfer failed");
		} else {
			// Only P3D and 3DPass ERC20 precompiles are supported on 3DPass
			revert("unsupported token type for transfers");
		}
	}
	...
	function receiveStakeAsset(uint stake_asset_amount) internal {
		if (isP3D(settings.tokenAddress)) {
			...
			require(msg.value == 0, "don't send P3D");
			require(IPrecompileERC20(settings.tokenAddress).transferFrom(msg.sender, address(this), stake_asset_amount), "3DPass ERC20 transferFrom failed");
		} else {
			// Only P3D and 3DPass ERC20 precompiles are supported on 3DPass
			revert("unsupported token type for staking");
		}
	}
